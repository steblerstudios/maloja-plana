...
| | |
|---|---|
| Aktueller Branch | `main` = `35bd840` (Sitzung 2026-07-14 Abend: Namens-Leak-Audit → `docs/archive` aus Repo (#86), Release-Härtung PII-CI + Auto-Tag (#87), Tag `v0.1.25-beta`, FTP-Secrets gelöscht, **Historie-Purge Personas/Tester/Freunde/Drittperson/Benutzername vollzogen**; Frontend in `maloja plana/maloja-frontend`) |
| `main` steht auf | `35bd840` — nach **Historie-Purge** (`filter-repo` + force-push 2026-07-14). Inhaltlich = alles bis #84 (live) plus nicht-App-Änderungen #86/#87 (Doku/CI/Tooling, Bundle unverändert). **Alle Hashes vor heute neu** (Purge). |
