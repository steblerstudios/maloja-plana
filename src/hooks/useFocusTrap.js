...
export const useFocusTrap = (aktiv, { ref, onEscape, neuAusrichten = [] } = {}) => {
  ...
  // Auslöser merken, beim Schliessen dorthin zurück. Bewusst NUR an `aktiv`
  // gehängt: hinge diese Rückkehr auch an `neuAusrichten`, spränge der Fokus bei
  // jedem Schritt eines mehrstufigen Dialogs kurz nach draussen und wieder herein
  // — der Screenreader läse dann jedes Mal den Knopf dahinter vor.
  useEffect(() => {
    const knoten = ref && ref.current;
    if (!aktiv || !knoten) return undefined;

    const vorher = document.activeElement;
    if (vorher && vorher.focus) ausloeser.current = vorher;
