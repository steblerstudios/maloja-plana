...
export const DirektLinks = ({ palette, t, data }) => {
  ...
  const renderLocalGov = () => {
    ...
    const links = [];
    if (city) { const cUrl = communeUrl(city); links.push(cUrl ? React.createElement(ExternerLink, { key: 'gm', t, href: cUrl, style: s.entryNameLink }, city) : React.createElement('span', { key: 'gm', style: s.entryName }, city)); }
    if (cantonName) {
      if (city) links.push(' · ');
  ...
  const renderBuch = () => {
    ...
    // Beratung & Hilfe: Beratungsstellen (help + die 3 von Stebler Studios) + Ombudsstellen (ombuds).
    if (offen === 'beratung') {
      const kids = [];
