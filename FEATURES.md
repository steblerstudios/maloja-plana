...
| **IPV-Lebenslinie Phase 2** (statefull Beleg: geschätzt→beantragt→bestätigt/Stempel→jährlich, additives `data.anspruch.ipv`) + a11y-Härtung (role='img' raus, Farbenblind-Balken) + Subsidiaritäts-Fix (IPV+Sozialhilfe nicht mehr doppelt) + Sozialhilfe-Rückerstattung (Copy belegt korrigiert + Rechner) | `built` | `main` via PR #60 (`91405d8`·`671da78`·`f987d06` + Predeploy-Fixes `b090feb`·`382042e`) | Tests 632 grün, `eslint src` sauber, Build ok, i18n-Parität. **In `main` (`d9ee62b`), NICHT deployt** — Predeploy-Runde 2 grün (5 branch-🔴 gefixt: Stempel/Kompass/Vanish/Total/Sie-Du), ZH-Rückerstattungsbeträge live gegen ZH-Handbuch 15.2.03 §27 SHG verifiziert. calculateIPV/-Sozialhilfe unberührt |
