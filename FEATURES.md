...
| **IPV-Lebenslinie Phase 2** (statefull Beleg: geschätzt→beantragt→bestätigt/Stempel→jährlich, additives `data.anspruch.ipv`) + a11y-Härtung (role='img' raus, Farbenblind-Balken) + Subsidiaritäts-Fix (IPV+Sozialhilfe nicht mehr doppelt) + Sozialhilfe-Rückerstattung (Copy belegt korrigiert + Rechner) | `built` | `main` via PR #60 (`91405d8`·`671da78`·`f987d06` + Predeploy-Fixes `b090feb`·`382042e`) | Tests grün, i18n-Parität. **In `main` (`a9578f1`), NICHT deployt** — Predeploy-Runde 2 grün (5 branch-🔴 gefixt: Stempel/Kompass/Vanish/Total/Sie-Du), ZH-Rückerstattungsbeträge live gegen ZH-Handbuch 15.2.03 §27 SHG verifiziert. calculateIPV/-Sozialhilfe unberührt |
| **AHV-21-Referenzalter der Frauen** (JG 1961–63 gestaffelt, monatsgenau, konsistent in Hauptergebnis + Vergleichstabelle + Zukunftsbild) + **Tresor-Lock-Fundament** (cryptoCore/secureStore, AES-256-GCM/PBKDF2, **dormant/unverdrahtet**) | `built` | `main` via PRs #62/#63 + Fixes #65/#66 (`a9578f1`) | Tests 651 grün, Build/Size ok, keine neuen Deps. **In `main`, NICHT deployt** — Predeploy-Runde 3 (19 Agenten) grün: AHV-Referenzalter swiss-precision-verifiziert (JG1962→2'341.33/0 %), Crypto-Primitive solide. **Tresor-🔴 (Dokumente unverschlüsselt etc.) als Phase-2b-Blocker geparkt** (`docs/design/tresor-lock.md`). Offen: AHV Phase B (Kürzungssätze). |
