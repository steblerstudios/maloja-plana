...
| `main` steht auf | **`5a4851c`** (= `origin/main`, gegengeprüft 2026-07-15). ⚠️ **Selbstauskunft zur Merge-Falle:** Dieser Eintrag ist der **Vor-Merge-Stand von PR #92**. Sein eigener Merge rückt `main` auf den Merge-Commit — das ist **kein Rückstand**, das Delta ist genau dieser eine Doku-Commit (kein `src/`, Bundle unverändert). Wer hier einen anderen `main`-Hash als unten vorfindet und **kein `src/`-Delta** sieht: alles in Ordnung, nur der Zeiger ist einen Merge alt. Gegenprobe statt Glauben: `git diff <hier>..main -- src/` → leer ⇒ live unverändert. Der frühere Eintrag `35bd840` war die **Merge-Falle**: Ein Stand-Sync-PR dokumentiert den Zeiger, sein eigener Merge rückt `main` dahinter. Delta `35bd840`→`5a4851c` = **nur Doku + Merge, kein `src/`** — bewiesen, indem `main` gebaut wurde: Bundle = `index-1f4c6867.js`, **identisch mit dem Live-Bundle**. |
