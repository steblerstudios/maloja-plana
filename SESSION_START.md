...
0. **Infomaniak-Panel: `Permissions-Policy` auf `geolocation=(self)` setzen** (Stebler Studios, Panel-Aktion). Aktuell `geolocation=()` → schaltet Geolocation origin-weit ab und **blockt den Opt-in-Standort der Notfall-Vorlesekarte**. Der r7-`.htaccess`-Fix greift NICHT (`deploy.sh` löscht `dist/.htaccess`, Zeile 111–114; Header laufen übers Panel). Danach live gegenprüfen: `curl -sD - -o /dev/null https://malojaplana.ch/ | grep -i permissions-policy`. Merker: das repo-`.htaccess` ist damit **vestigial** — Header nur im Panel pflegen (oder `.htaccess` mit Kommentar entschärfen). Optional: `0.1.25-beta` taggen.
